...
class ServerRenderer {
  ...
  constructor(bundle: any, template: any, stats: any) {
    this.template = template;
    this.stats = stats;
    this.serverEntry = this._createEntry(bundle);
  }

  renderToString(request: any, staticContext: any) {
    return new Promise((resolve, reject) => {
      ...
      const render = () => {
        ...
        if (context.statusCode) {
          ...
          resolve({
            error: undefined,
            html: this._generateHTML(root, extractor, store.getState(), css),
          });
        }
      };
      ...
      promises = matchs.map(({ route, match }: any) => {
  ...
  _generateHTML(root: string, extractor: any, initalState: any, css: any) {
